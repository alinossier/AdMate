...
public func rounder (numbertoround: CGFloat) -> String {
...
//            self.print("viewDidChangeBackingProperties, new scale = \(window!.backingScaleFactor)")
        ...
        override func mouseMoved(theEvent: NSEvent) {
            ...
            let mouseLocation = lineChart.convertPoint(theEvent.locationInWindow, toLayer: lineChart.datasets.first)
            ...
            if let point = lineChart.closestPointTo(mouseLocation) {
                if popoverPoint == nil || popoverPoint! != point {
                    ...
                    popover.contentSize = NSSize(width: 75.0, height: 45.0)
                    ...
                    popover.showRelativeToRect(rect, ofView: self, preferredEdge: NSRectEdge.MaxY)
    ...
    class SimpleLineChartTooltop : NSViewController {
        ...
        let lineLabel = NSTextField(frame: CGRect(x: 0.0, y: 20.0, width: 75.0, height: 20.0))
        let valueLabel = NSTextField(frame: CGRect(x: 0.0, y: 0.0, width: 75.0, height: 20.0))
        ...
        override init?(nibName nibNameOrNil: String?, bundle nibBundleOrNil: NSBundle?) {
            ...
            self.view = NSView(frame: CGRect(x: 0.0, y: 0.0, width: 75.00, height: 45.0))
        ...
        override var representedObject: AnyObject? {
            didSet {
                if let point = representedObject as? LineChart.Point {
                    ...
                    if point.IndiceLabel == "%" {
                    
                        valueLabel.stringValue = "\(ConvertPercentage(Float(point.value)))" + " \(point.IndiceLabel)"
                        lineLabel.stringValue = point.PopLabel
                    
                    } else {
                        
                        valueLabel.stringValue = "\(rounder(point.value))" + " \(point.IndiceLabel)"
                        lineLabel.stringValue = point.PopLabel
    ...
    var datasets: [Dataset] = [Dataset]() {
        didSet {
            ...
            for dataset in datasets {
                ...
                if !yAxes.contains(dataset.yAxis) {
                    yAxes += [dataset.yAxis]
                }
...
//        addSublayer(yAxes.first!) // here
    ...
    override func layoutSublayers() {
        ...
        for yAxis in yAxes {
            ...
            let minYValue = datasetsForAxis.reduce(CGFloat.max, combine: { min($0, ($1.data).minElement()!) })
            let maxYValue = datasetsForAxis.reduce(CGFloat.min, combine: { max($0, ($1.data).maxElement()!) })
...
extension LineChart {
    ...
    class Axis : CALayer {
        ...
        func optimalInterval(min: CGFloat, max: CGFloat) -> CGFloat {
            ...
            let magnitude = pow(10.0, round( log10(minInterval) ))
    ...
    class Dataset : CAShapeLayer {
        ...
        convenience init(label: String, data: [CGFloat], yAxis: Axis, poplabelfrominit: [String], indice: String) {
            self.init(label: label, data: data, poplabelfrominit: poplabelfrominit, indice: indice)
            self.yAxis = yAxis
        }
        init(label: String, data: [CGFloat], poplabelfrominit: [String], indice: String) {
            self.label = label
            self.data = data
            self.PopLabels = poplabelfrominit
            self.indice = indice
            ...
            defaultPoint.addObserver(self, forKeyPath: "strokeColor", options: NSKeyValueObservingOptions(), context: nil)
            defaultPoint.addObserver(self, forKeyPath: "fillColor", options: NSKeyValueObservingOptions(), context: nil)
            defaultPoint.addObserver(self, forKeyPath: "path", options: NSKeyValueObservingOptions(), context: nil)
        ...
        deinit{
            defaultPoint.removeObserver(self, forKeyPath: "strokeColor")
            defaultPoint.removeObserver(self, forKeyPath: "fillColor")
            defaultPoint.removeObserver(self, forKeyPath: "path")
        }
        ...
        func updatePoints() {
            ...
            for var i = 0; i < data.count; i++ {
                ...
                point.value = data[i]
                point.PopLabel = PopLabels![i]
                point.IndiceLabel = indice
    ...
    func closestPointTo(refPoint: CGPoint) -> Point? {
        ...
        for dataset in datasets {
            for point in dataset.points {
                ...
                // Values -220 and + 30 are completely arbitrary and depend on the chart's view size. Not sure what to do about that besides hard coding them
                
                let xDist = abs(refPoint.x - point.position.x - 220)
                let yDist = abs(refPoint.y - point.position.y + 30)
...
extension CGPath {
    ...
    class func SplineCurve(points: [CGPoint], tension: CGFloat = 0.3, minY: CGFloat = CGFloat.min, maxY: CGFloat = CGFloat.max) -> CGPath {
        func controlPoints(t: CGPoint, i: CGPoint, e: CGPoint, s: CGFloat) -> (inner: CGPoint, outer: CGPoint) {
            ...
            let n = sqrt(pow(i.x - t.x, 2) + pow(i.y - t.y, 2));
            let o = sqrt(pow(e.x - i.x, 2) + pow(e.y - i.y, 2))
            let a = s * n / (n + o)
            let h = s * o / (n + o)
            let inner = CGPoint(x: i.x - a * (e.x - t.x), y: i.y - a * (e.y - t.y))
            let outer = CGPoint(x: i.x + h * (e.x - t.x), y: i.y + h * (e.y - t.y))
            return (inner, outer)
        }
        ...
        for var i = 0; i < points.count; i++ {
            ...
            var controlPoints = controlPoints(prevPoint, i: point, e: nextPoint, s: pTension)
...
extension CGAffineTransform {
    init(verticalFlipWithHeight height: CGFloat) {
        let t = CGAffineTransformMakeScale(1.0, -1.0)
    ...
    var attributedString : NSAttributedString {
        if let attString = string as? NSAttributedString  {
            ...
            if layerFont == nil {
                ...
                layerFont = CTFontCreateUIFontForLanguage(CTFontUIFontType.System, fontSize, nil)
                
            }
            //return NSAttributedString(string: string as NSString, attributes: [NSFontAttributeName: layerFont!])
            return NSAttributedString(string: string as NSString as String, attributes: [kCTFontAttributeName as String: layerFont!])
        } else {
